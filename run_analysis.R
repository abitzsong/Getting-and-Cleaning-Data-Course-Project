...
colnames(DATA) <- c("subject","activity",features$activityNames)
...
DATAselected <- select(DATA,subject,activity,select.colnames)

#Uses descriptive activity names to name the activities
descriptiveLabels <- activity_labels$X2
DATAselected$activity <- as.factor(DATAselected$activity)
levels(DATAselected$activity) <- descriptiveLabels
...
library(reshape2)
DATAmelted <- melt(DATAselected,c("activity","subject"),measure.vars = select.colnames, value.name = "value")
DATAtidy <- dcast(DATAmelted, subject + activity ~ variable, mean)

#creating a txt tidy dataset for submission
DATAtidy2 <- arrange(DATAtidy,subject,activity)
